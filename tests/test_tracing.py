...
from thinharness.tracing import ModelTraceSnapshot, _SpanAdapter, annotate_model_request, serialize_attribute_value
...
def test_create_langfuse_tracing_uses_env_and_legacy_header(monkeypatch) -> None:
    calls = []

    def fake_create_otlp_tracing(**kwargs):
        calls.append(kwargs)
        return "tracing"
    ...
    monkeypatch.setattr("thinharness.tracing.create_otlp_tracing", fake_create_otlp_tracing)

    assert create_langfuse_tracing(service_name="svc", legacy_ingestion=True) == "tracing"

    assert calls[0]["endpoint"] == "https://eu.cloud.langfuse.com/api/public/otel/v1/traces"
    assert calls[0]["headers"]["Authorization"] == "Basic cGs6c2s="
    assert calls[0]["headers"]["x-langfuse-ingestion-version"] == "4"

    calls.clear()
    create_langfuse_tracing(service_name="svc", legacy_ingestion=False)
    assert "x-langfuse-ingestion-version" not in calls[0]["headers"]

    monkeypatch.delenv("LANGFUSE_PUBLIC_KEY")
    with pytest.raises(RuntimeError, match="LANGFUSE_PUBLIC_KEY"):
        create_langfuse_tracing(service_name="svc")
