...
- RESUME-2: `resume_state` is self-contained and does not depend on provider continuation tokens such as OpenAI `previous_response_id`; an OpenAI run that never received a response id is still resumable. An OpenAI replay-mode envelope additionally carries the exact raw Responses item history in the optional `openai_items` key (see OPENAI-STATE-4).
...
- RESUME-4: Built-in provider resume state uses `version` 5. User and tool entries carry canonical ordered content blocks; image bytes are stored once as base64. A tool entry also stores nullable `wire_output` when its exact provider-facing text differs from the canonical result, and replay uses that text byte-for-byte. The optional `openai_items` key is present only in envelopes dumped by OpenAI replay-mode sessions; Anthropic, OpenRouter, and OpenAI continuation-mode envelopes omit it. Older state and old provider-native `kind` values are rejected with a regenerate error. An approval envelope that contains an older provider state fails with `approval state provider_state version <n> is not supported`.
...
The OpenAI Responses adapter manages conversation state on the client by default. It replays the exact ordered raw Responses items on every request, so the harness matches the client-managed defaults of comparable harnesses and does not depend on server-side response storage. Server-side `previous_response_id` continuation remains available as an explicit opt-in.

### Requirements

- OPENAI-STATE-1: `OpenAIResponsesModel(..., state_mode=...)` accepts `"replay"` (default) or `"continuation"`; any other value raises `ValueError`. `HarnessConfig` and `infer_model` do not expose the mode; continuation requires constructing the model directly.
- OPENAI-STATE-2: In replay mode every request sends `store: false`, never sends `previous_response_id`, and sends `input` as a list containing the complete ordered raw item history followed by the new items. Every user turn is a `message` item; replay mode never sends scalar text input.
- OPENAI-STATE-3: The raw item history holds the exact input items the adapter sent (user messages, `function_call_output` items with byte-exact `wire_output` when present) and every `response["output"]` item exactly as received, including reasoning `encrypted_content`, item `id`, `call_id`, `status`, and message `phase`. `output_text` never becomes an item.
- OPENAI-STATE-4: Replay-mode `dump_state()` includes the raw item history as `openai_items`. Resume on an OpenAI replay-mode model uses `openai_items` verbatim when the history contains no `reasoning` item or the resuming model is reasoning-capable; otherwise it renders the neutral `entries` per RESUME-3. Cross-provider resume, continuation-mode captures, and Anthropic or OpenRouter resume use `entries` and ignore `openai_items`.
- OPENAI-STATE-5: Crash resume, approval resume, and forks from the same state produce the same next request as an uninterrupted run. Resuming does not mutate the state dict, and two sessions resumed from one state do not share history.
- OPENAI-STATE-6: In replay mode `start(..., previous_response_id=...)` raises `ProviderError`. Continuation mode keeps its prior behavior: scalar text input, `previous_response_id` chaining after the first response, no `store` key, one-time neutral transcript replay on resume, and an envelope without `openai_items`.
- OPENAI-STATE-7: In replay mode `extra_body` may not contain `input`, `previous_response_id`, or `store`; construction raises `ValueError`.
- OPENAI-STATE-8: In replay mode a response containing a `reasoning` item without `encrypted_content` raises `ProviderError` before any later request; the error names the remedies (extend the reasoning-model detection list or use continuation mode). Reasoning items are never silently dropped.
- OPENAI-STATE-9: `openai_items` is validated before any session is created or provider call is made. Rejected with a `resume_from`-prefixed `HarnessError`: a non-dict element or missing string `type`; a `function_call_output` with no earlier unanswered `function_call` of the same `call_id`; a `function_call` still unanswered when a later user `message` or `reasoning` item appears; a `reasoning` item that is last or immediately followed by a user `message` or `function_call_output`. Trailing unanswered `function_call` items are accepted because approval pauses dump that shape. Through the approval path the prefix is `approval state provider_state`.
- OPENAI-STATE-10: No code path retries a request through the other state mode, including after a `cyber_policy` refusal.
...
- PLUGIN-11: The parent child host builds one ordered unique agent catalog from recipe names registered during synchronous plugin binding. Blank names fail, while repeated names across delegation tools share one catalog entry. Core seals registration after all plugins bind and before validating agent-filtered hooks; registration during connection or execution fails without changing delegation provenance, recipes, or names.
- PLUGIN-11A: A delegation tool registered during binding contributes its recipe names to the static catalog even when the tool itself is contributed during connection. Connected contributions cannot change the catalog. A runtime child request must use a name in the sealed catalog.
...
- BASH-PLUGIN-5: Stdout and stderr are drained concurrently into separate bounded head-and-tail buffers. Each buffer retains `ceil(limit / 2)` bytes from the start and `floor(limit / 2)` bytes from the end. For each truncated stream, the result reports the omitted byte count, zero-based half-open retained ranges, drain completion, and a root-relative path to a separate binary artifact containing every captured stream byte in order. An artifact is called complete only when its reader reached EOF; an escaped descendant can hold a pipe open past the bounded final drain, in which case the result labels the artifact as bytes captured before the drain cutoff.
- BASH-PLUGIN-5A: Bash creates overflow artifacts lazily under `.thinharness/outputs/` and does not create them for untruncated streams. Artifact disk use grows with total truncated output, is not bounded by `max_output_bytes`, and can be large up to the host timeout. A returned artifact remains until an external caller removes it or the harness root; the host must reap artifacts it no longer needs. Cancellation performs best-effort removal of unpublished artifacts. An artifact persistence failure performs best-effort partial-file removal, remains visible in the result without hiding command exit or timeout metadata, and makes an otherwise successful result fail.
...
- SUBAGENTS-PLUGIN-10: `ChildHarnessRequest.tool_mode` is the sole child-inheritance state: `"inherited"` selects default or inherited-only composition, `"inherited+explicit"` selects additive inherited and explicit sources, and `"explicit"` disables parent inheritance. Hook contexts and delegation result metadata expose the boolean inheritance value derived from that mode.
...
- PROVIDER-SETTINGS-2: A directly constructed `AnthropicMessagesModel(max_tokens=...)` overrides `ModelSettings.max_tokens`; a top-level `extra_body["max_tokens"]` overrides both because tuning keys are applied before `extra_body`. OpenAI replay mode reserves the `extra_body` keys `input`, `previous_response_id`, and `store` and rejects them at model construction (OPENAI-STATE-7).
